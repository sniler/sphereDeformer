...
MStatus SphereDeformer::deform(
    ...
    for (unsigned int i = 0; i < numberOfPoints; i++)
    {
        ...
        if (v.length() < 1.0)
        {
            float weight = this->weightValue(dataBlock, geoIndex, i);
            MPoint deformedPoint = MPoint(v.normal()) * sphereMatrix * worldInverseMatrix;

            MVector delta = deformedPoint - inPoint;

            MPoint p(points[i] + (delta * envelope_ * weight));
            points.set(i, p.x, p.y, p.z);
        }
    }
